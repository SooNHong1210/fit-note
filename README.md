...
2. **DB 스키마** — [supabase/migrations/](./supabase/migrations) 에 마이그레이션으로 관리.
   - 자동: `supabase/migrations/**` 를 main에 push하면 GitHub Action이 `supabase db push`로 적용
     (아래 "DB 자동 동기화" 참고)
   - 수동(최초/로컬): `supabase db push --db-url <연결문자열>`,
     또는 SQL Editor에서 마이그레이션 파일 내용을 순서대로 실행
   - 빠른 시연용 느슨한 RLS가 필요하면 [db/rls-demo.sql](./db/rls-demo.sql) 수동 실행
     (⚠️ 회원 데이터 비보호), 초기화는 [db/reset.sql](./db/reset.sql)
3. Supabase 대시보드 설정:
   - **Authentication → Providers → Email**: "Confirm email" 끄기(데모)
   - **Authentication → Anonymous sign-ins** 켜기 (회원 인증에 필요)
...
### DB 자동 동기화 (GitHub Action)
`supabase/migrations/**` 변경을 push하면 [.github/workflows/supabase.yml](./.github/workflows/supabase.yml) 이
`supabase db push` 로 프로덕션 DB에 적용합니다.
- GitHub 시크릿 `SUPABASE_DB_URL` 필요 (Settings → Database → Connection string →
  **Session pooler**(IPv4) URI, 비밀번호 포함)
- 최초 1회는 수동 push 후 이력이 잡히면, 이후 변경은 새 마이그레이션 파일을 추가해 push
...
> **보안 현황:** 마이그레이션의 `rls_secure` 적용 시 선생님은 자기 샵만, 회원은
> 자기 데이터만 접근(회원은 익명인증). 잔여 노출: 회원은 같은 샵의 예약/수업 *시간*은
> 볼 수 있음(슬롯 계산용). [db/rls-demo.sql](./db/rls-demo.sql) 은 시연 전용으로 회원 데이터를 보호하지 않습니다.
...
supabase/
  ...
  migrations/        DB 마이그레이션(스키마·RLS·realtime) — CI 자동 적용
db/
  reset.sql          테이블 초기화(수동, 파괴적)
  rls-demo.sql       시연용 느슨한 RLS(수동, 비보호)
```
